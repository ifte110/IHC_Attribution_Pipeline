...
def fetch_data(conn, table_name, start_date=None, end_date=None):
    try:
        query = f"SELECT * FROM {table_name}"
        if start_date and end_date and 'event_date' in pd.read_sql_query(f"PRAGMA table_info({table_name})", conn)['name'].values:
...
def populate_channel_reporting(conn, start_date=None, end_date=None):
    try:
        query = """
        ...
        if start_date and end_date:
            query += " WHERE ss.event_date BETWEEN ? AND ?"

        query += " GROUP BY ss.channel_name, ss.event_date HAVING SUM(acj.ihc) <= 1.0;"

        if start_date and end_date:
            conn.execute(query, (start_date, end_date))
        else:
            conn.execute(query)
...
try:
    ...
    if api_key:
        api_responses = send_to_api(customer_journeys[:200], api_url, api_key)
        ihc_data = extract_ihc_data(api_responses)
        insert_ihc_data(conn, ihc_data)
        populate_channel_reporting(conn)

        if start_date and end_date:
            output_file = f'channel_reporting_{start_date}_to_{end_date}.csv'
        else:
            output_file = 'channel_reporting_full.csv'

        export_channel_reporting_to_csv(conn, output_file)
